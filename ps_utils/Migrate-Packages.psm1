...
function UploadCli {
    ...
    $args = "-verb:sync -source:contentPath='$cliPath' -dest:contentPath='$target',ComputerName='https://$publishUrl/msdeploy.axd?site=$site',UserName='$userName',Password='$password',AuthType='Basic' -enableRule:DoNotDeleteRule"

    Write-Host "Deploying Cli"

    $proccess = Start-Process $msdeployPath -ArgumentList $args -Wait -NoNewWindow -PassThru

    if ($proccess.ExitCode) {
        throw "Failed to deploy Cli."
    }
}
...
function Start-PackagesMigration {
    ...
    Add-Content $jobFile """%WEBROOT_PATH%\Tools\Cli\UiPath.Orchestrator.Cli.exe"" packages start^"
...
function Finalize-PackagesMigration {
    ...
    $jobFilePath = [System.IO.Path]::GetTempPath()
    $jobFile = New-Item -Path (Join-Path $jobFilePath "run.cmd") -Force
    Add-Content $jobFile """%WEBROOT_PATH%\Cli\UiPath.Orchestrator.Cli.exe"" packages finish^"
    Add-Content $jobFile " --connection-string ""$connectionString"""
