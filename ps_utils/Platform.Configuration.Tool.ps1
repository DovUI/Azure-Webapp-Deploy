...
param (
    ...
    [Parameter(ValueFromPipeline = $true)] [string] $siteName = "UiPath Orchestrator",
    # New Orchestrator Url to update.
    [Parameter(Mandatory = $false)] [string] $newOrchestratorUri,
    # New Identity Server URL if it is not in the /identity.
    [Parameter(Mandatory = $false)] [string] $newIdentityServerUrl,
    # SQL connection string when Orchestrator web.config is encrypted
    [Parameter(Mandatory = $false)] [string] $sqlConnectionString,
    # New Token Signing Thumbprint to update.
    [Parameter(Mandatory = $false)] [string] $newTokenSigningThumbprint,
    # New SSL Thumbprint to update.
    [Parameter(Mandatory = $false)] [string] $newSSLThumbprint
)

Import-Module $PSScriptRoot\Platfrom.Configuration.Tool.psm1

if ($newOrchestratorUri) {
    (Update-UiPathUrl $siteName $newOrchestratorUri $newIdentityServerUrl $sqlConnectionString)
}

if ($newTokenSigningThumbprint -or $newSSLThumbprint) {
    (Update-UiPathCertificate $siteName $newTokenSigningThumbprint $newSSLThumbprint)
}
...
$userAccess = $certPkAccess | Where-Object { $_.IdentityReference.Value -like "*$appPoolUser" }
...
[URI]$newOrchestratorUri = $identityProductionConfig.AppSettings.OrchestratorUrl
$orchestratorHost = $newOrchestratorUri.DnsSafeHost

if (!$sslCert) {
    Write-Error "No certificate found at $sslCertPath." -Category ObjectNotFound
}
elseif (-not ($sslCert.DnsNameList | where-Object { $orchestratorHost -like $_ })) {
