param(
    [ValidateScript({ if (-Not ($_ | Test-Path -PathType Leaf)) {throw "The resource catalog service package file path parameter ( -package ) is not valid."} return $true })]
...
function DisplayException($ex) {
    Write-Host ($ex | Format-List -Force | Out-String)
}
